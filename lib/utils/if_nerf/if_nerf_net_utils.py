...
def init_smpl(smpl):
    ...
    for i in range(cfg.ni):
        smpl_path = os.path.join(smpl_dir, f"{i + 1}.ply")
...
def record_mask_depth(output, batch):
    ...
    msk_path = os.path.join("data/train_mask_depth", "mask", f"{img_path[:-4]}.png")
    depth_path = os.path.join("data/train_mask_depth", "depth", f"{img_path[:-4]}.png")
