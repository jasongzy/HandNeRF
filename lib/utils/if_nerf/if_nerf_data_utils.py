...
from lib.utils.img_utils import enlarge_box
...
def sample_ray_h36m(img, msk, K, R, T, bounds, nrays, split, forbidden_mask=None):
    H, W = img.shape[:2]
    ray_o, ray_d = get_rays(H, W, K, R, T)
    ...
    assert msk.any(), "No foreground object according to the mask"

    if forbidden_mask is None:
        forbidden_mask = np.zeros_like(msk)
    ...
    if split == "train":
        ...
        while nsampled_rays < nrays:
            ...
            # sample rays on body
            coord_body = np.argwhere((msk == 1) & (forbidden_mask != 1))
            if len(coord_body) <= n_body:  # make sure that samples cover the whole body
                coord_body = np.concatenate(
                    (coord_body, coord_body[np.random.randint(0, len(coord_body), n_body - len(coord_body))])
                )
            else:
                coord_body = coord_body[np.random.randint(0, len(coord_body), n_body)]
            # sample rays on face
            coord_face = np.argwhere((msk == 13) & (forbidden_mask != 1))
            if len(coord_face) > 0:
                coord_face = coord_face[np.random.randint(0, len(coord_face), n_face)]
            # sample rays in the bound mask
            coord = np.argwhere((bound_mask == 1) & (forbidden_mask != 1))
        ...
        # reduce the amount of pixels to render according to the mask
        near_full = np.zeros_like(msk, dtype=near.dtype)
        far_full = np.zeros_like(msk, dtype=far.dtype)
        near_full[mask_at_box.reshape(H, W)] = near
        far_full[mask_at_box.reshape(H, W)] = far
        box = enlarge_box(cv2.boundingRect(msk), ratio=1.5, img_shape=img.shape[:2])
        msk[box[1] : box[1] + box[3], box[0] : box[0] + box[2]] = 1
        mask_at_box[~msk.flatten().astype(bool)] = False
        near = near_full[mask_at_box.reshape(H, W)]
        far = far_full[mask_at_box.reshape(H, W)]
...
def get_rigid_transformation(poses, joints, parents, return_joints=False):
    ...
    padding = np.zeros([joints.shape[0], 1, 4])
    padding[..., 3] = 1
    ...
    padding = np.zeros([joints.shape[0], 1])
...
def get_bounds(xyz, box_padding=cfg.box_padding):
...
def get_radii(H, W, K, R, T):
    """For Mip-NeRF

    Returns:
        radii: [H, W]
    """
    _, directions = get_rays(H, W, K, R, T)
    directions = directions.astype(np.float32)
    dx = np.sqrt(np.sum((directions[:-1, :, :] - directions[1:, :, :]) ** 2, axis=-1))
    dx = np.concatenate([dx, dx[-2:-1, :]], axis=-2)
