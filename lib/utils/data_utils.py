...
def select_point(shape, poly, gt_mask):
    for _ in range(cfg.max_iter):
        ...
        if inter_from_polys(poly_move, shape[1], shape[0], gt_mask):
            return x, y
    x, y = -1, -1
    return x, y
...
def read_mano_uv_obj(filename):
    ...
    vt, ft, f = (
        np.array(vt, dtype="float64"),
        np.array(ft, dtype="int32") - 1,
        np.array(f, dtype="int32") - 1,
    )  # Neural Actor encoder.py line 1241
...
from fairnr.clib._ext import point_face_dist_backward, point_face_dist_forward
...
def get_uvd(pts, mesh_vertices, mesh_faces, mesh_face_uv):
    ...
    triangles = F.embedding(
        mesh_faces, mesh_vertices
    )  # inputs: indices, lookup_table          #torch.Size([1538, 3, 3])
    l_idx = torch.tensor(
        [
            0,
        ]
