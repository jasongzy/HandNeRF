...
class Dataset(data.Dataset):
    def __init__(self, data_root, human, ann_file, split, **kwargs):
        ...
        self.ims = np.array(
            [
                ...
                for ims_data in annots["ims"][i : i + cfg.num_train_frame * cfg.frame_interval]
            ]
        )
        ...
        self.Ts = np.array(self.cams["T"])[cfg.training_view].astype(np.float32) / 1000.0
    ...
    def prepare_input(self, i):
        ...
        vertices_path = os.path.join(self.data_root, cfg.vertices, f"{i}.npy")
        wxyz = np.load(vertices_path).astype(np.float32)

        # transform smpl from the world coordinate to the smpl coordinate
        params_path = os.path.join(self.data_root, cfg.params, f"{i}.npy")
    ...
    def __getitem__(self, index):
        ...
        R = cv2.Rodrigues(Rh)[0].astype(np.float32)
        meta = {"R": R, "Th": Th, "latent_index": latent_index, "frame_index": frame_index}
        ret.update(meta)
