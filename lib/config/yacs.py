...
class CfgNode(dict):
    ...
    def to_hparams_dict(self):
        ...
        hparams_dict = _to_dict(self)
        hparams_dict = flatten_dict(hparams_dict)
        for k, v in list(hparams_dict.items()):
            if not (isinstance(v, (bool, str, float, int)) or v is None):
...
def _load_cfg_py_source(filename):
    ...
    _assert_with_logging(
        ...
        "Imported module 'cfg' attr must be in {} but is {} instead".format(VALID_ATTR_TYPES, type(module.cfg)),
    )
