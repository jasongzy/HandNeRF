...
class Evaluator:
    ...
    def evaluate(self, output, batch):
        ...
        tgt_mesh_path = os.path.join(data_root, f"object/{frame_index:06d}.obj")
        tgt_mesh = o3d.io.read_triangle_mesh(tgt_mesh_path)
        tgt_mesh = trimesh.Trimesh(tgt_mesh.vertices, tgt_mesh.triangles, process=False)
    ...
    def summarize(self):
        ...
        print(f"p2s: {np.mean(self.p2ss)}")
        print(f"chamfer: {np.mean(self.chamfers)}")
...
class MeshEvaluator:
    ...
    def apply_registration(self):
        transform, _ = trimesh.registration.mesh_other(self.src_mesh, self.tgt_mesh)
