...
@torch.compile(mode="reduce-overhead", disable=not cfg.torch_compile)
def raw2outputs(raw, z_vals, white_bkgd=False) -> torch.Tensor:
    ...
    weights = (
        alpha
        * torch.cumprod(torch.cat([torch.ones((alpha.shape[0], 1)).to(alpha), 1.0 - alpha + 1e-10], -1), -1)[:, :-1]
    )
    rgb_map = torch.sum(weights[..., None] * rgb, -2)  # [N_rays, 3]

    depth_map = torch.sum(weights * z_vals, -1)
    disp_map = 1.0 / torch.max(1e-10 * torch.ones_like(depth_map).to(depth_map), depth_map / torch.sum(weights, -1))
...
def sample_pdf(bins, weights, N_samples, det=False):
    ...
    if det:
        u = torch.linspace(0.0, 1.0, steps=N_samples).to(cdf)
...
def get_intersection_mask(sdf, z_vals):
    ...
    ind = torch.min(sign * torch.arange(sign.size(2)).flip([0]).to(sign), dim=2)[1]
...
def sphere_tracing(wpts, sdf, z_vals, ray_o, ray_d, decoder):
    ...
    """
    sign = torch.sign(sdf[..., :-1] * sdf[..., 1:])
    ind = torch.min(sign * torch.arange(sign.size(1)).flip([0]).to(sign), dim=1)[1]
    ...
    while unfinished_mask.sum() != 0 and i < 20:
        ...
        unfinished_mask[unfinished_mask] = (mid_sdf.abs() > sdf_threshold) | (mid_sdf < 0)
