...
class PositionalEncoder(nn.Module):
    ...
    @property
    def out_dim(self):
        return (self.in_dim if self.append_identity else 0) + self.in_dim * 2 * (self.max_deg - self.min_deg)
...
class IntegratedPositionalEncoder(nn.Module):
    ...
    def forward(self, x_coord: torch.Tensor):
        ...
        if self.diag:
            ...
            num_dims = x.shape[-1]
            basis = torch.cat(
                [2**i * torch.eye(num_dims, device=x.device) for i in range(self.min_deg, self.max_deg)],
                1,
            )
    ...
    def _expected_sin(self, x, x_var):
        ...
        y_var = torch.clip(0.5 * (1 - torch.exp(-2 * x_var) * torch.cos(2 * x)) - y**2, min=0)
        return y, y_var
    ...
    def forward(self, x: torch.Tensor, alpha: float):
        ...
        if self.append_identity:
            identity, features = torch.split(
                features, (self.in_dim, self.in_dim * 2 * (self.max_deg - self.min_deg)), dim=-1
            )
        features = features.reshape(list(x.shape[:-1]) + [self.max_deg - self.min_deg, self.in_dim, 2])
        window = self.cosine_easing_window(alpha).reshape((self.max_deg - self.min_deg, 1, 1)).to(features)
        features = window * features
        if self.append_identity:
            return torch.cat([identity, features.reshape(list(x.shape[:-1]) + [-1])], dim=-1)
        else:
            return features
...
class MipNerfModel(nn.Module):
    ...
    def forward(self, rays, color_bkgd, randomized: bool = True, **kwargs):
        ...
        for i_level in range(self.num_levels):
            ...
            rgb, density = self._query_mlp(rays, samples, randomized=randomized, **kwargs)
...
def cast_rays(t_vals, origins, directions, radii, ray_shape, diag=True) -> tuple[torch.Tensor, torch.Tensor]:
...
def sample_along_rays(
    ...
    Args:
        ...
        radii: torch.ndarray(float32), [batch_size, 1], ray radii.
        num_samples: int.
...
def resample_along_rays(
    ...
    means, covs = cast_rays(new_t_vals, origins, directions, radii, ray_shape, diag)
...
def sorted_piecewise_constant_pdf(bins, weights, num_samples, randomized):
    ...
    if randomized:
        ...
        u = u + s * torch.rand(list(cdf.shape[:-1]) + [num_samples], device=cdf.device)
        # `u` is in [0, 1) --- it can be zero, but it can never be 1.
        u = torch.clip(u, max=1.0 - torch.finfo().eps)
    else:
        # Match the behavior of jax.random.uniform() by spanning [0, 1-eps].
        u = torch.linspace(0.0, 1.0 - torch.finfo().eps, num_samples, device=cdf.device)
