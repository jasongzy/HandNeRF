...
def main(args):
    ...
    assert len(img_paths_gt) == len(img_paths_lq), (
        "GT folder and LQ folder should have the same length, but got " f"{len(img_paths_gt)} and {len(img_paths_lq)}."
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input",
        nargs="+",
        default=["datasets/HandNeRF/ROM02_complete_gt", "datasets/HandNeRF/ROM02_complete"],
    ...
    parser.add_argument(
        "--meta_info",
        type=str,
        default="datasets/HandNeRF/meta_info/meta_info_ROM02_complete.txt",
